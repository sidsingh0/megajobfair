...
if ((isset($_GET["phone"])) and (isset($_GET["dob"]))) {
    ...
    $query2="select a.*,c.company_name,c.room from allotments a, companies c where student_id='$phone' and a.company_id=c.id order by timestamp";
    if (mysqli_num_rows($query_res)<1){
        ...
        $query_res=$query_res->fetch_assoc();
        echo '{"status":1,"aptitude":'.$query_res["apti_marks"].',"attendance":'.$query_res["attended"].'}';
        }
}}else{
